...
timestamps2 <-  as.POSIXct(c("Tue Aug 17 15:59:33 1999 +0000","Tue Aug 17 16:59:33 1999 +0000",
                            "Tue Aug 17 17:59:33 1999 +0000","Tue Aug 17 18:59:33 1999 +0000"),
                          format = "%a %b %d %H:%M:%S %Y %z", tz = "UTC")


project_git2 <- data.table(file_pathname = c("file_a","file_a","file_a","file_a"),
                          author_name_email = c("dev 1","dev 2","dev 2","dev 3"),
                          committer_name_email = c("dev 1","dev 2","dev 2","dev 3"),
                          author_datetimetz = timestamps2,
                          committer_datetimetz = timestamps2,
...
test_that("temporal projection window matches original formulation", {

  #git_graph <- transform_gitlog_to_bipartite_network(project_git,"author-file")
  git_graph <- copy(project_git2)
  setnames(git_graph,
           old = c("author_name_email",
                   "file_pathname"),
           new = c("from",
...
test_that("temporal projection window matches original formulation with alternating authors", {
