test_that("download_bugzilla_rest_issues() returns correct error with incorrect bugzilla site", {
  ...
  start_timestamp <- strftime(as.POSIXlt(Sys.time() - 60, "UTC"), "%Y-%m-%dT%H:%M:%S%z")
  issues_folder_path <- "/tmp/bugzilla_rest_issues/"
  suppressWarnings({
    error <- system2('mkdir',
...
test_that("download_bugzilla_rest_comments() returns correct error with incorrect bugzilla site", {
  ...
  start_timestamp <- strftime(as.POSIXlt(Sys.time() - 86400, "UTC"), "%Y-%m-%dT%H:%M:%SZ")
  issues_folder_path <- path.expand("/tmp/bugzilla_rest_comments/")
  comments_folder_path <- path.expand("/tmp/bugzilla_rest_issues/")
  suppressWarnings({
    error <- system2('mkdir',
...
test_that("download_bugzilla_rest_issues_comments() returns correct error with incorrect bugzilla site", {
  incorrect_bugzilla_site = "https://incorrect_bugzilla_123123.com"
  start_timestamp <- strftime(as.POSIXlt(Sys.time() - 86400, "UTC"), "%Y-%m-%dT%H:%M:%SZ")
  issues_comments_folder_path <- path.expand("/tmp/bugzilla_rest_issues/")
  suppressWarnings({
    error <- system2('mkdir',
                     args = c(issues_comments_folder_path),
                     stdout = TRUE,
                     stderr = FALSE)
  })
