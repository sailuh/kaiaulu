...
A hardcoded function for folder creation to store the API responses:
...
for (i in 1:length(openhub_portfolio_projects[["name"]])) {
  ...
  openhub_api_iterate_pages(token, openhub_api_projects, project_folder_path, openhub_api_parameters, max_pages=1)
}
```
...
matchedProjectsCount <- 1
while (matchedProjectsCount <= nrow(openhub_portfolio_projects)) {
  project_name <- openhub_portfolio_projects[["name"]][[matchedProjectsCount]]
  openhub_api_parameters[["project_name"]] <- project_name
  for (i in 1:length(projects_api_requests)) {
    openhub_projects[[matchedProjectsCount]] <- openhub_parse_projects(projects_api_requests[[i]], openhub_api_parameters)
    if (nrow(openhub_projects[[matchedProjectsCount]]) != 0) {
      matchedProjectsCount <- matchedProjectsCount + 1
      break
    }
  }
}
openhub_projects <- unique(rbindlist(openhub_projects), by = "name")
gt(openhub_projects)
```
...
```{r, eval = FALSE}
openhub_combined_projects <- unique(merge(openhub_projects, openhub_portfolio_projects, by = "name", all = FALSE), by = "name")
...
for (i in 1:length(openhub_combined_projects[["name"]])) {
  ...
  openhub_api_iterate_pages(token, openhub_api_analyses, analysis_folder_path, openhub_api_parameters)
}
```
...
openhub_analyses <- unique(rbindlist(openhub_analyses), by = "id")
gt(openhub_analyses)
```
