...
# Filters
file_extensions <- conf[["heuristics"]][["keep_files_ending_with"]]
substring_filepath <- conf[["heuristics"]][["filter_filepaths_containing"]]
```
...
```{r}
project_git_slice <- project_git[data.AuthorDate >= as.POSIXct("2019-01-01", format = "%Y-%m-%d",tz = "UTC") & data.AuthorDate < as.POSIXct("2019-01-31", format = "%Y-%m-%d",tz = "UTC")]
...
```{r}
project_commit_message_id_network <- parse_commit_message_id_network(project_git,commit_message_id_regex = cveid_regex)

project_commit_message_id_network <- igraph::graph_from_data_frame(d=project_commit_message_id_network[["edgelist"]], 
                      directed = TRUE, 
                      vertices = project_commit_message_id_network[["nodes"]])
