---
title: "Anna Karenina Study Project Showcase"
output: 
  html_document:
    toc: true
...
vignette: >
  ...
  %\VignetteIndexEntry{Anna Karenina Study Project Showcase}
...
```{r eval = FALSE}
# You can use this code block to obtain the start and end commit hash according to the 
# time window of interest
# max(project_git[year(author_datetimetz) == 2005]$author_datetimetz)
# "2004-12-31 14:35:40 UTC"
# "2004-01-01 09:55:09 UTC"
#end <- ymd_hms("2005-12-21 22:44:54")
#start <- ymd_hms("2005-01-02 22:33:45")
#project_git[author_datetimetz == end]$commit_hash
```
...
```{r}
# This function will calculate flaws for a given commit hash
...
# Create a list where each element is the dv8 flaws calculated for a given commit hash
flaws <- list()
all_windows <- length(time_window)
for(j in 2:length(time_window)){
  i <- j - 1
  print(paste0("Window N.: ",i," and ",j," out of: ",all_windows))
  ...
  commit_interval <- stri_c(i_commit_hash,"-",j_commit_hash)
  flaws[[commit_interval]] <- dv8_flaws(script_path, tools_path, conf_path)
  
  
  # Arch Report generate more columns than we need. Specify flaws column and add commit hash to it
  flaws[[commit_interval]] <- flaws[[commit_interval]][,.(commit_interval,
                                                          git_checkout = j_commit_hash,
                                                          FileName,
                                                          dv8numClique = numClique,
                                                          dv8numCrossing = numCrossing,
                                                          dv8numModularityViolation = numModularityViolation,
                                                          dv8numPackageCycle = numPackageCycle,
                                                          dv8numUnhealthyInheritance = numUnhealthyInheritance,
                                                          dv8numUnstableInterface = numUnstableInterface,
                                                          dv8LOC=as.numeric(LOC),
                                                          dv8Fanin = FanIn,
                                                          dv8FanOut = FanOut)]
  # Filter Files
  flaws[[commit_interval]] <- flaws[[commit_interval]]  %>%
  filter_by_file_extension(file_extensions,"FileName")  %>% 
  filter_by_filepath_substring(substring_filepath,"FileName")
}
# Reset Repo to HEAD
git_checkout(branch,git_repo_path)

flaws_files <- rbindlist(flaws)
flaws_interval <- flaws_files[,.(dv8numClique = sum(dv8numClique),
                                 dv8numCrossing = sum(dv8numCrossing),
                                 dv8numModularityViolation = sum(dv8numModularityViolation),
                                 dv8numPackageCycle = sum(dv8numPackageCycle),
                                 dv8numUnhealthyInheritance = sum(dv8numUnhealthyInheritance),
                                 dv8numUnstableInterface = sum(dv8numUnstableInterface),
                                 dv8LOC=sum(dv8LOC),
                                 dv8Fanin = sum(dv8Fanin),
                                 dv8FanOut = sum(dv8FanOut)),
                              by = c("commit_interval","git_checkout")]
```
...
dt <- merge(dt,flaws_interval,by="commit_interval")
...
```{r eval = FALSE}
fwrite(dt,"~/Desktop/smells_flaws_tomcat.csv")
```
