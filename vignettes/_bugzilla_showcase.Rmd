...
conf <- yaml::read_yaml("../conf/redhat.yml")
...
We can use 'download_bugzilla_rest_issues_comments' function to download both issues and issues with comments from the Bugzilla site.

These 3 website will work with the downloader, however the parser does not work.

https://gcc.gnu.org/
https://bugzilla.mozilla.org/
https://bugzilla.yoctoproject.org/

More details at: https://github.com/kaiaulu/issues/285
...
```{r eval = FALSE}
download_bugzilla_rest_issues_comments_by_date(bugzilla_site, start_timestamp, save_issues_path_redhat, key, limit_upperbound=20, comments=FALSE, verbose=TRUE)
```

```{r}
bugzillarest_issues_table <- parse_bugzilla_rest_issues(save_issues_path_redhat)
head(bugzillarest_issues_table)  %>%
  gt(auto_align = FALSE)
```
...
```{r}
bugzillarest_issues_comments_table <- parse_bugzilla_rest_issues_comments(save_issues_comments_path_redhat)
head(bugzillarest_issues_comments_table)  %>%
  gt(auto_align = FALSE) 
```

```{r eval = FALSE}
refresh_bugzilla_issues_comments(bugzilla_site, start_timestamp = start_timestamp, save_folder_path = save_issues_comments_path_redhat, project_key = key, limit_upperbound=20, comments = TRUE, verbose = TRUE)
```
