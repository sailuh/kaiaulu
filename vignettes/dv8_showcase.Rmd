...
Our first step is to parse the Git log and Dependencies for DV8. These will used to construct historical and structural dependencies in DV8. 
...
```{r}
numstat_path <- dv8_gitlog_to_gitnumstat(git_repo_path,file.path(project_path,"apr-numstat.txt"))
hdsmb_path <- dv8_gitnumstat_to_hdsmb(dv8_path,numstat_path,hdsmb_path = file.path(project_path,"apr-hdsm.dv8-dsm"))
```
...
sdsmj_path <- dv8_depends_to_sdsmj(depends_jar=depends_jar_path,
                                   ...
                                   language = language,
                                   sdsmj_path = file.path(project_path,"apr-sdsm.json"))
sdsmb_path <- dv8_dsmj_to_dsmb(dv8_path = dv8_path,
                                     dsmj_path = sdsmj_path, 
                                     dsmb_path = file.path(project_path,"apr-sdsm.dv8-dsm"))
...
mdsmb_path <- dv8_hdsmb_sdsmb_to_mdsmb(dv8_path=dv8_path,
                                         ...
                                         mdsmb_path = file.path(project_path,"apr-merge.dv8-dsm"))
list.files(project_path)
```
...
hierclsxb_path <- dv8_mdsmb_to_hierclsxb(dv8_path = dv8_path,
                                          ...
                                          hierclsxb_path = file.path(project_path,"apr-clsx.dv8-clsx"))
list.files(project_path)
```
...
mdsmb_xlsx <- dv8_mdsmb_drhier_to_excel(dv8_path = dv8_path,
                                         mdsmb_path = mdsmb_path,
                                         excel_path = file.path(project_path,"apr-mdsm.xlsx")#,
...
flaws_folder <- dv8_mdsmb_to_flaws(dv8_path = dv8_path, 
                                    ...
                                    historyImpact=flaws_params$historyImpact,
...
We can see a folder of Architectural Flaws was generated. The folder organization provides us with means to understand what files are assigned to various types of architectural flaws. For example:
...
hierclsxj_path <- dv8_clsxb_to_clsxj(dv8_path,
                                     ...
                                     clsxj_path = file.path(project_path,"apr-clsxj.json"))
...
dl_metrics <- dv8_mdsmb_to_decoupling_level(dv8_path = dv8_path,
                                             mdsmb_path = mdsmb_path,
                                             json_path = file.path(project_path,"apr-dl.json"))
...
# Gitlog and Dependencies with Filters

This section will describe milestone 3.4 `gitlog_to_hdsmj` and `dependencies_to_sdsmj`, Kaiaulu functions which can write DV8 compatible dsmj. 
