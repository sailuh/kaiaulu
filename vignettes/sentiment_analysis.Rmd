...
The input is a CSV of labeled Github commit comments from [sailuh/sentiment_github_dataset](https://github.com/sailuh/sentiment_github_dataset/blob/4ad05b680d7870308539b6e41e916d6b9077c16f/data/commit%20comments/akka_commit_comments.csv):
```{r}
train_df <- fread("https://github.com/user-attachments/files/27574331/akka_sentiment_commit_comments_joined.csv")
```
...
The input is a CSV of unlabeled Kaiaulu PR inline comments. Running pysenti_predict on this file will attach a predicted sentiment polarity label to each comment.
```{r}
reply_dt <- fread("https://github.com/user-attachments/files/27574385/kaiaulu_pr_inline_comments.csv")
reply_dt <- setnames(reply_dt, "body", "text")
reply_dt[, polarity := 0] # Add polarity column placeholder
```
