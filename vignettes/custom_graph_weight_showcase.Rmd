...
head(dt[, .(comment_id, issue_url, comment_user_login, polarity)]) %>%
  gt(auto_align = FALSE)
```
...
head(dt[, .(reply_from, reply_subject, polarity_int)]) %>%
  gt(auto_align = FALSE)
```
...
head(reply_graph[["nodes"]]) %>%
  gt(auto_align = FALSE)
head(reply_graph[["edgelist"]]) %>%
  gt(auto_align = FALSE)
```
...
head(emotion_dt[, .(comment_id, issue_key, author_login, author_name, love, joy, sadness, anger, surprise, fear)]) %>%
  gt(auto_align = FALSE)
```
...
head(emotion_dt[, .(reply_from, reply_subject, love, joy, sadness, anger, surprise, fear)]) %>%
  gt(auto_align = FALSE)
```
