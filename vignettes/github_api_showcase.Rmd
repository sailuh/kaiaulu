...
save_pr_review <- get_github_pr_review_path(conf, "project_key_1")
...
Next we download pull request review data from GitHub API. This will be used to know of the status of each review: APPROVED....

```{r Collect all project commit messages, eval = FALSE}
gh_response <- github_api_pr_reviews(owner,repo,pull_number)
github_api_iterate_pages(token,gh_response,save_pr_review,prefix="pr_reviews")
```
