...
repo <- "perceive" # Has to match github repository (e.g. github.com/sailuh/perceive)
save_path <- "~/Desktop/" # Path you wish to save all raw data. A folder with the repo name and sub-folders will be created.
...
```{r Collect all issue events}
gh_response <- github_api_project_issue(owner,repo,token)
save_path_issue_event <- paste0(save_path,repo,"/issue_event/")
dir.create(save_path_issue_event)
github_api_iterate_pages(gh_response,save_path_issue_event,prefix="issue_event")
```
...
```{r Collect all project commit messages}
gh_response <- github_api_project_commits(owner,repo,token)
save_path_commit <- paste0(save_path,repo,"/commit/")
dir.create(save_path_commit)
github_api_iterate_pages(gh_response,save_path_commit,prefix="commit")
```
...
all_issue_event <- lapply(all_issue_event,github_parse_project_issue)
