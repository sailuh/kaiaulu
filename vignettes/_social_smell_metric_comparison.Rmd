...
for(j in 2:size_time_window){
  ...
  if(gitlog_exist){
    ...
    igraph_git_network_authors <- igraph::bipartite_projection(igraph_network_git_slice,
                                            multiplicity = TRUE,
    ...
    igraph.code.clusters <- walktrap.community(igraph_git_network_authors,
                                  weights=E(igraph_git_network_authors)$weight)
  ...
  if(ml_exist){
    ...
    igraph_mbox_network_authors <- igraph::bipartite_projection(igraph_network_mbox_slice,
                                          multiplicity = TRUE,
                                          which = TRUE) # FALSE is the thread projection
    # Community Detection
    ...
    igraph.mail.clusters <- walktrap.community(igraph_mbox_network_authors,
                                      weights=E(igraph_mbox_network_authors)$weight)
  ...
  if(ml_exist){
    ...
    igraph_radio_silence <- length(community_smell_radio_silence(mail.graph=igraph_mbox_network_authors, 
                                                          clusters=igraph.mail.clusters))
  ...
  if (ml_exist & gitlog_exist){
    ...
    igraph_org_silo <- length(community_smell_organizational_silo(mail.graph=igraph_mbox_network_authors,
                                                          code.graph=igraph_git_network_authors))

    missing_links <- length(smell_missing_links(mbox_network_authors,
                                                git_network_authors))
        
    igraph_missing_links <- length(community_smell_missing_links(igraph_mbox_network_authors,
                                                                 igraph_git_network_authors))
    # Social Technical Metrics
...
#    communicability <- community_metric_mean_communicability(mbox_network_authors,git_network_authors)
  ...
  smells[[commit_interval]] <- data.table(commit_interval,
                                          ...
                                          igraph_org_silo,
                                          missing_links,
                                          igraph_missing_links,
                                          radio_silence,
                                          igraph_radio_silence,
                                          #primma_donna,
                                          #st_congruence,
                                          #communicability,
                                          num_tz,
...
We can also inspect a given slice to assess the community detection algorithm for fine tunning. For example, the last time slice for both git log and mailing list is as follows:
