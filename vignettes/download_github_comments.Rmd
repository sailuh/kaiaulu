...
save_path_issue_refresh <- paste0(save_path,"/refresh/")
...
Note in the subsequent code block we specified the fields from the issue we are interested in downloading. Again, you may specify [any fields](https://docs.github.com/en/search-github/searching-on-github/searching-issues-and-pull-requests#search-by-when-an-issue-or-pull-request-was-created-or-last-updated) of interest for the downloader, including custom fields of your Github instance, but the parser currently only support those listed here. 

```{r Collect issues by date, eval = FALSE}
date_upper_bound_issue=NULL
date_lower_bound_issue="2024-02-07"
# make initial API CALL
gh_response <- github_api_project_issue_by_date(owner,
                                                repo,
                                                token,
                                                date_lower_bound_issue,
                                                date_upper_bound_issue,
                                                verbose=TRUE)
dir.create(save_path_issue)
# Make subsequent API calls and write to JSON file along save path
github_api_iterate_pages(token,gh_response,
...
## Collect issue comments by lower bound date

```{r Collect issues by date, eval = FALSE}
date_lower_bound_comment="2024-02-07"
# make initial API CALL
gh_response <- github_api_project_issue_or_pr_comments_by_date(owner,
                                                repo,
                                                token,
                                                date_lower_bound_comment,
                                                verbose=TRUE)
dir.create(save_path_issue_or_pr_comments)
# Make subsequent API calls and write to JSON file along save path
github_api_iterate_pages(token,gh_response,
...
Similar to the refresh of the issues, this chunk allows for the downloading of comments that have been created since the most recently created date among data already downloaded. This allows us to 'refresh' the comments, downloading comments made since that date or continue downloading if a rate limit was reached. 
```{r Collect all issues, eval = FALSE}
#gh call but with date
# get the data
...
## Refresh parser
The Refresh parser also parses issue data but the folder 'refresh_issues'
```{r}
