...
If using a personal account token from a free GitHub account, the number of API calls per hour increases to 5000 **per hour**. Furthermore, using the GraphQL API will require a token to perform any query or mutation. Therefore, it is recommended you create a personal token by following the [GitHub Documentation instructions](https://docs.github.com/en/free-pro-team@latest/github/authenticating-to-github/creating-a-personal-access-token#:~:text=Creating%20a%20token.%201%20Verify%20your%20email%20address%2C,able%20to%20see%20the%20token%20again.%20More%20items). The process should not take more than 2 minutes.

Once you've created a personal account token, you will need to ensure that it is placed within `~/.ssh/github_token`. Note that the .ssh folder is normally hidden, so it may already exist on your local device. The `github_token` file must not contain any file extentions, and will contain only the token.
...
You will also require the file paths to the location you wish to save the rawdata.
This can be done manually, or they can be obtained from the parsed configuration file of the project of interest.
...
# Create all folder directories on local device
...
Therefore, in this Notebook we have to rely on four endpoints from the GitHub REST API: The `Issue endpoint` to obtain the "first comment" of every issue, the `Pull Request endpoint` to obtain the "first comment" of every pull request, followed by the `Issue and Pull Request Comment endpoint`, which provides comments for both issue and pull requests together.

This notebook additionally provides an endpoint from the Github GraphQL API to obtain comments from Discussions Posts and their replies.
