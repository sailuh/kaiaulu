...
#placeholder function to test refresh
```{r}
...
#REFRESH Issues
```{r}
# get the filename of the latest date
...
```{r}
# Example usage
issue_most_recent_created_date <- format_created_at_from_file(latest_date_issue)
print(issue_most_recent_created_date)
```
```{r Collect all issues, eval = FALSE}
#gh call but with date
#created <- paste0('>',most_recent_created_date)
gh_response <- github_api_project_issue_refresh(owner,repo,token,issue_most_recent_created_date)
dir.create(save_path_issue)
github_api_iterate_pages(token,gh_response,
                         save_path_issue,
                         prefix="issue")
...
```{r}
latest_date <- paste0(save_path_issue, parse_jira_latest_date(save_path_issue))
message(latest_date)
pr_most_recent_created_date <- format_created_at_from_file(latest_date)
print(most_recent_created_date)
```
