...
```{r Collect Reviews from Pull Requests, eval = FALSE}
...
The last endpoint downloads the inline code comments. This table provides direct association between a comment and a specific line on the file being modified. This table offers therefore a bridge between communication and code changes not found in any other data source. As noted in the introduction, inline comments that are associated to a change request will contain a `review_id` value which can be inner joined to the pull request review table above.
