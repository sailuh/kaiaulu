...
In GitHub, a Pull Request conversation starts off with the Pull Request description. Pull Request comments are made by users. A reviewer can start a review where they can add a single comment, request changes, or approve for merging. This starts the review comment. From there, in-line code comments can be made from the review.
...
gh_response <- github_api_pr_reviews_refresh(owner, repo, token, save_path_pull_request, save_path_pr_reviews,verbose=TRUE)
```
