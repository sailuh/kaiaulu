---
title: "Exploring the Rolling Window Metrics"
output: 
  html_document:
    toc: true
    number_sections: true
vignette: >
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteIndexEntry{Exploring the Rolling Window Metrics}
  %\VignetteEncoding{UTF-8}
---

# Introduction

This notebook presents some of the functionality of the following functions from `metric.R` that measure developer engagement: `engagement_communication()`, `productivity_author_commits()`, `productivity_author_churn()`, and `jira_developer_communication_count()`.

The `engagement_sentiment()` metric is covered separately in `sentiment_analysis.Rmd`, as it requires an additional pipeline to obtain the data.

```{r}
rm(list = ls())
seed <- 1
set.seed(seed)
```

```{r warning=FALSE,message=FALSE}
require(kaiaulu)
require(data.table)
require(yaml)
require(stringi)
require(knitr)
require(magrittr)
require(ggplot2)
require(lubridate)
```

# Project Configuration File

Analyzing open source projects often requires some manual work on your part to find where the open source project hosts its codebase and mailing list. Instead of hard-coding this on Notebooks, we keep this information in a project configuration file. Here's the minimal information this Notebook requires in a project configuration file:

```
data_path:
  project_website: https://apr.apache.org/
  git_url: https://github.com/apache/apr
  git: ../rawdata/git_repo/APR/.git
  
filter:
  keep_filepaths_ending_with:
    - cpp
    - c
    - h
    - java
    - js
    - py
    - cc
  remove_filepaths_containing:
    - test
```

As you can see, the project configuration file is a simple bullet list. This is by design: We want the files to be human readable, so you can share by e-mail, include as appendix, or even attach as supplemental material in a conference. To facilitate formatting and commenting on files, we use .YAML, instead of plain .txt or markdown. 

What this file tells the R Notebook is where to find the git log on your computer, and where you got it from in the first place. Currently we don't really use `project_website` nor `git_url` but it is strongly encouraged, as we have encountered in the past projects with multiple mirrors where the git log contained **discrepancies**, making reproducing prior analysis much harder.

In the project configuration file we also specify filters. They tell this notebook what to keep after the `.` in a filename (i.e. what it ends with), and what it should **not** keep based on any word within the entire filepath name. For example, in APR unit tests are prefix with the word `_test`. In trying to reproduce related work, we found neglecting file filters led metrics such as `churn` blow out of proportion, for it include many nonsensical file changes.

The file makes all assumptions explicit to you when using the code. Note these assumptions are not universal: They are particular to this project alone. This is why this lives in a project configuration file, instead of the codebase. Kaiaulu git repository `/conf` folder includes a few existing projects with that information. The idea is that we save time on the long run without having to look again on the project website manually.

The following code block reads the information explained just now:

```{r}
tool <- parse_config("../tools.yml")
conf <- parse_config("../conf/kaiaulu.yml")
perceval_path <- get_tool_project("perceval", tool)
git_repo_path <- get_git_repo_path(conf)
git_branch <- get_git_branches(conf)[1]
nvdfeed_folder_path <- get_nvdfeed_folder_path(conf)

# Filters
file_extensions <- get_file_extensions(conf)
substring_filepath <- get_substring_filepath(conf)
```

This is all the project configuration files are used for. If you inspect the variables above, you will see they are just strings. As a reminder, the tools.yml is where you store the filepaths to third party software in your computer. Please see Kaiaulu's README.md for details. As a rule of thumb, any R Notebooks in Kaiaulu load the project configuration file at the start, much like you would normally initialize variables at the start of your source code.

# Engagement Metric

## Obtaining the Output Table
We can obtain commit comments using downloader function `github_api_project_commit_comments()` utilizing the GitHub API. We then parse the output of the functionnusing `github_parse_project_commit_comments()` to obtain a data table with information on commit commnets for a given project. Here, we show an example of how to obtain commit comments for the Kaiaulu project itself. You can change the `owner` and `repo` variables to obtain commit comments for any other public GitHub repository. Note that you will need a GitHub token with the appropriate permissions to access the API.
```{r}
owner <- "sailuh"
repo <- "kaiaulu"
token <- Sys.getenv("token")
```

```{r}
github_api_project_commit_comments <- function(owner, repo, token){
  gh::gh("GET /repos/{owner}/{repo}/comments",
         owner = owner,
         repo = repo,
         page = 1,
         per_page = 100,
         .token = token)
}
```

```{r}
github_parse_project_commit_comments <- function(api_responses){
  parse_response <- function(api_response){
    parsed_response <- list()
    parsed_response[["comment_id"]] <- api_response[["id"]]
    parsed_response[["commit_id"]] <- api_response[["commit_id"]]
    parsed_response[["author_login"]] <- api_response[["user"]][["login"]]
    parsed_response[["author_id"]] <- api_response[["user"]][["id"]]
    parsed_response[["body"]] <- api_response[["body"]]
    parsed_response[["created_at"]] <- api_response[["created_at"]]
    parsed_response[["updated_at"]] <- api_response[["updated_at"]]

    parsed_response <- as.data.table(parsed_response)

    return(parsed_response)
  }
  rbindlist(lapply(api_responses, parse_response), fill = TRUE)
}
```

We can save the output to a variable called `comments_dt` and convert the timezones from the `created_at` variable to the same timezone. Here, we use UTC.
```{r}
comments_dt <- github_parse_project_commit_comments(
  github_api_project_commit_comments(owner, repo, token)
)
```

```{r}
comments_dt[, created_at := as.POSIXct(
  created_at,
  format = "%Y-%m-%dT%H:%M:%SZ", 
  tz = "UTC"
  )]
```

We'll show the head of the output table for the function below.
```{r}
kable(head(comments_dt))
```

Here, we will define the rolling window as 90 days (around 3 months), which is the default value for the `quit_lag` parameter in the `engagement_communication()` function. If you want to change the value of lag, you can do so by passing a different value in the function call. Afterwards, we will call `engagement_communication()` to obtain the output table and save it to a variable called `engagement_communication_output`. We pass in the relevant columns containing the timestamp, the user of a comment, as well as the value of the lag. The output table will contain the number of messages sent by each user in the past 90 days at each point in time, which is one way to  measure their engagement with the project. Note that the `engagement_communication()` function can be used with any type of communication data, such as issue comments or pull request comments.
```{r}
quit_lag = 90
```

```{r}
engagement_communication_output <- engagement_communication(
  datetimetz = comments_dt$created_at,
  user_name_email = comments_dt$author_login,
  quit_lag = quit_lag
)
```
```{r}
data.table::setnames(
  engagement_communication_output,
  make.unique(names(engagement_communication_output))
)
```

```{r}
kable(head(engagement_communication_output))
```

## Visualizing the Output Tables as a Time Series

To visualize the output tables as a time series, we can use the `ggplot2` package. Visualizing the engagement metrics as a time series is useful in obersving the trends of engagement of an author over time though the number of messages they have sent.

```{r fig.width=12, fig.height=7}
ggplot(engagement_communication_output, aes(x = datetimetz, y = message_count, color = user_name_email)) +
  geom_line() +
  labs(title = paste0("Author Engagement on ", quit_lag, "-day Time Window"),
       x = "Date",
       y = "Number of Messages",
       color = "Author"
       ) +
       theme_minimal() +
       theme(plot.title = element_text(hjust = 0.5))
```


# Productivity Metrics

## Obtaining the Git Log
Our first step is to parse the git log. Many of the variables in Kaiaulu are tables, which makes it easier to export and manually inspect the data at any step of the analysis.

```{r}
git_checkout(git_branch,git_repo_path)
project_git <- parse_gitlog(perceval_path,git_repo_path)
#project_git <- parse_gitlog(perceval_path,git_repo_path,save_path)
#project_git <- readRDS(save_path)
```

## Filter files

We may also want to filter files, to include only source code and exclude test files for example. 

```{r}
project_git <- project_git  %>%
  filter_by_file_extension(file_extensions,"file_pathname")  %>% 
  filter_by_filepath_substring(substring_filepath,"file_pathname")
```

## Obtaining the Output Tables

We must first convert all timestamps to the same timezone. Here, we use UTC.

```{r}
project_git$author_datetimetz <- as.POSIXct(
  project_git$author_datetimetz,
  format = "%a %b %d %H:%M:%S %Y %z",
  tz = "UTC"
)
```

Now, we can obtain the output tables from `productivity_author_commits` and `productivity_author_churn`. We will save them to a variable called `productivity_author_commits_output` and `productivity_author_churn_output` respectively and keep the default value of lag to be 90 days (around 3 months) to define the rolling window. If you want to change the value of lag, you can do so by passing a different value in the function call. For example, if you want to use a 120-day (around 4 months) rolling window, you can set `lag = 120` in the function call.

```{r}
lag = 90
```

```{r}
productivity_author_commits_output <- productivity_author_commits(project_git, lag = lag)
productivity_author_churn_output <- productivity_author_churn(project_git, lag = lag)
```

We'll show the head of the output tables for the two functions below.
```{r}
kable(head(productivity_author_commits_output))
kable(head(productivity_author_churn_output))
```

## Visualizing the Output Tables as a Time Series

To visualize the output tables as a time series, we can use the `ggplot2` package. Visualizing the productivity metrics as a time series is useful in observing the engagement of an author over time through the number of messages they send and their churn. One way to think of it is that in conjunction with the engagement metrics, we can use the productivity metrics to understand if an author is becoming more or less engaged over time with their sentiment scores reflecting such increase or decrease in engagement.

### Plotting the Time Series for the Top Authors

We'll select the top 10 authors based on their number of commits and churn, and then plot the time series for these authors.

```{r}
top_n_authors <- 10
```

The timeseries below shows the rolling window for author productivity by unique commits.

```{r fig.width=12, fig.height=7}
# Compute the top authors based on unique commits
top_authors_commits <- productivity_author_commits_output[
    ,.(max_commits = max(author_total_commits, na.rm = TRUE)),
    by = author_name_email
    # Sort max commits in descending order and select the top n authors
][order(-max_commits)][1:top_n_authors, author_name_email]

# Select only the rows corresponding to the top authors based on unique commits
plot_commits_dt <- productivity_author_commits_output[
    author_name_email %in% top_authors_commits
    ][order(author_name_email, author_datetimetz)]

ggplot(plot_commits_dt, aes(x = author_datetimetz, y = author_total_commits, color = author_name_email)) +
    geom_line() +
    labs(title = paste0("Author Productivity on ", lag, "-day Time Window"),
         x = "Date",
         y = "Number of Commits",
         color = "Author"
         ) +
         theme_minimal() +
         theme(plot.title = element_text(hjust = 0.5))
```

The timeseries below shows the rolling window for author productivity by churn.
```{r fig.width=12, fig.height=7}
# Compute the top authors based on churn
top_authors_churn <- productivity_author_churn_output[
    ,.(max_churn = max(author_churn, na.rm = TRUE)),
    by = author_name_email
    # Sort max churn in descending order and select the top n authors
][order(-max_churn)][1:top_n_authors, author_name_email]

# Select only the rows corresponding to the top authors based on churn
plot_churn_dt <- productivity_author_churn_output[
   author_name_email %in% top_authors_churn
   ][order(author_name_email, author_datetimetz)]


ggplot(plot_churn_dt, aes(x = author_datetimetz, y = author_churn, color = author_name_email)) +
    geom_line() +
    labs(title = paste0("Author Productivity on ", lag, "-day Time Window"),
         x = "Date",
         y = "Churn",
         color = "Author"
         ) +
         theme_minimal() +
         theme(plot.title = element_text(hjust = 0.5))
```

# Communication Metric

## Minimal Example Shaped Like the Comments Table Returned by `parse_jira()`
The communication metric `jira_developer_communication_count()` is designed to work with data in the same format as the comments table returned by `parse_jira()`. This means that the input data table should have columns for issue key, comment ID, comment creation timestamp, comment author, and comment body. Below is a minimal example of how to create a data table with this format to test the `jira_developer_communication_count()` function.
```{r}
comments_dt <- data.table(
  issue_key = c(
    "PROJ-1","PROJ-1","PROJ-1",
    "PROJ-2","PROJ-2","PROJ-2",
    "PROJ-3","PROJ-3","PROJ-3",
    "PROJ-4","PROJ-4"
  ),
  comment_id = c(
    "10001","10002","10003",
    "20001","20002","20003",
    "30001","30002","30003",
    "40001","40002"
  ),
  comment_created_datetimetz = c(
    # 2022
    "2022-01-15T10:00:00.000+0000",
    "2022-03-20T12:30:00.000+0000",
    "2022-11-05T09:10:00.000+0000",

    # 2023
    "2023-02-10T14:05:10.000+0000",
    "2023-06-01T08:45:00.000+0000",
    "2023-12-18T16:20:00.000+0000",

    # 2024
    "2024-01-08T11:00:00.000+0000",
    "2024-04-22T19:10:00.000+0000",
    "2024-09-30T07:55:00.000+0000",

    # 2025
    "2025-02-14T13:40:00.000+0000",
    "2025-10-03T21:15:00.000+0000"
  ),
  comment_author_name = c(
    "Alice Doe <alice@example.com>",
    "Alice Doe <alice@example.com>",
    "Alice Doe <alice@example.com>",
    "John Smith <john@example.com>",
    "John Smith <john@example.com>",
    "John Smith <john@example.com>",
    "Alice Doe <alice@example.com>",
    "Mary Major <mary@example.com>",
    "Mary Major <mary@example.com>",
    "John Smith <john@example.com>",
    "Alice Doe <alice@example.com>"
  ),
  comment_body = c(
    "Initial report",
    "Can reproduce",
    "Still happening in latest release",
    "Investigating",
    "Patch posted for review",
    "Confirmed fixed on main",
    "Regression found",
    "Any updates?",
    "I tested the fix; looks good",
    "Follow-up: additional edge case",
    "Closing the loop on this issue"
  )
)
```

## Run the Metric

```{r}
lag <- 90
```

```{r}
out <- jira_author_communication_count(
  comments_dt,
  comment_created_col = "comment_created_datetimetz",
  comment_author_col = "comment_author_name",
  lag = lag
)
```

```{r}
out[order(comment_author_name, datetimetz)]
```

## Visualizing the Output Table as a Time Series

We can plot the output table as a time series using `ggplot2`. This will allow us to observe the trends of communication of different authors over time through the number of comments they have sent, particularly in Jira. This can be useful in understanding how different authors are engaging with a project over time, and if there are any patterns in their communication behavior. For example, we may observe that certain authors become more active during certain periods, or that there are fluctuations in communication around major releases or events. You can adjust the `lag` parameter above to see how the communication trends change with different rolling window sizes.

```{r fig.width=12, fig.height=7}
plot_dt <- as.data.table(out)
plot_dt <- plot_dt[, !duplicated(names(plot_dt)), with = FALSE]

ggplot(plot_dt, aes(x = datetimetz, y = comment_count, color = comment_author_name)) +
  geom_line() +
  labs(title = paste0("Author Communication on ", lag, "-day Time Window"),
       x = "Date",
       y = "Number of Comments",
       color = "Author"
       ) +
       theme_minimal() +
       theme(plot.title = element_text(hjust = 0.5))
```


# Comparing the Time Series of Different Engagement Metrics

We can compare the time series of an author for all of the metrics across the rolling window. To do this, we will select one author and normalize the values of the metrics metrics to be on the same scale (0-1) using min-max normalization. Then, we will plot the metrics on the same graph to compare their trends over time.

```{r fig.width=12, fig.height=7}
one_author <- "Carlos Paradis <carlosviansi@gmail.com>"

# Filter to one author
commits_dt <- productivity_author_commits_output[
  author_name_email == one_author,
  .(author_datetimetz, author_total_commits)
][order(author_datetimetz)]

churn_dt <- productivity_author_churn_output[
  author_name_email == one_author,
  .(author_datetimetz, author_churn)
][order(author_datetimetz)]

engagement_dt <- engagement_communication_output[
  user_name_email == one_author,
  .(datetimetz, message_count)
][order(datetimetz)]
engagement_dt[, author_datetimetz := datetimetz]
engagement_dt[, datetimetz := NULL]

communication_dt <- out[
  comment_author_name == one_author,
  .(datetimetz, comment_count)
][order(datetimetz)]
communication_dt[, author_datetimetz := datetimetz]
communication_dt[, datetimetz := NULL]

# Join on timestamp so all metrics share the same x-axis
plot_dt <- merge(commits_dt, churn_dt, by = "author_datetimetz", all = TRUE)
plot_dt <- merge(plot_dt, engagement_dt, by = "author_datetimetz", all = TRUE)
plot_dt <- merge(plot_dt, communication_dt, by = "author_datetimetz", all = TRUE)


# Normalize the values using min-max normalization to bring them to a 0-1 scale
normalize <- function(x) {
  if (!is.numeric(x)) return(x)
  if (all(is.na(x))) return(x)

  x_min <- min(x, na.rm = TRUE)
  x_max <- max(x, na.rm = TRUE)

  if (x_max == x_min) return(rep(0, length(x)))

  (x - x_min) / (x_max - x_min)
}

plot_dt_normalized <- as.data.table(lapply(plot_dt, normalize))

# Make long table for plotting all metrics
plot_long <- rbindlist(list(
  plot_dt_normalized[, .(author_datetimetz, metric = "Commits (normalized)", value = author_total_commits)],
  plot_dt_normalized[, .(author_datetimetz, metric = "Churn (normalized)",   value = author_churn)],
  plot_dt_normalized[, .(author_datetimetz, metric = "Engagement (normalized)", value = message_count)],
  plot_dt_normalized[, .(author_datetimetz, metric = "Communication (normalized)", value = comment_count)]
))

ggplot(plot_long, aes(x = author_datetimetz, y = value, color = metric)) +
  geom_line() +
  labs(
    title = paste0("Commits and Churn Productivity on ", lag, "-day Time Window (Normalized): ", one_author),
    x = "Date",
    y = "Normalized value (0–1)",
    color = "Metric"
  ) +
  theme_minimal() +
  theme(plot.title = element_text(hjust = 0.5))
```