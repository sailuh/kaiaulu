...
Here, we will define the rolling window as 90 days (around 3 months), which is the default value for the `lag` parameter in the `engagement_communication()` function. If you want to change the value of lag, you can do so by passing a different value in the function call. The output table will contain the number of messages sent by each developer in the past 90 days at each point in time, which is one way to  measure their engagement with the project. Note that the `engagement_communication()` function can be used with any type of communication data, such as issue comments or pull request comments.
...
# For sparse rolling window data, filter to authors with multiple observations
authors_with_multiple_windows <- engagement_communication_output[
  , .N, by = author_login
][N > 1, author_login]

plot_data <- engagement_communication_output[author_login %in% authors_with_multiple_windows]

if (nrow(plot_data) > 0) {
  ggplot(plot_data, aes(x = timestamp, y = message_count, color = author_login)) +
    geom_point() +
    geom_line() +
  ...
  message("Note: No authors have multiple complete rolling windows in the data.")
  message("Extend the date range or reduce the lag parameter for multiple windows per author.")
  
  # Show all data points even if only one per author
  ggplot(engagement_communication_output, aes(x = timestamp, y = message_count, color = author_login)) +
    geom_point(size = 3) +
    labs(title = paste0("Author Engagement on ", lag, "-day Rolling Window (Single Windows)"),
         x = "Date",
         y = "Number of Messages",
         color = "Author"
         ) +
...
top_authors_commits <- productivity_author_commits_output[
    ,.(max_commits = max(author_total_commits, na.rm = TRUE), n_windows = .N),
    by = author_name_email
    # Sort max commits in descending order and select the top n authors
][order(-max_commits)][1:min(top_n_authors, .N), author_name_email]
...
if (nrow(plot_commits_dt) > 0) {
  ...
  message("No data available for top authors.")
}
```
...
top_authors_churn <- productivity_author_churn_output[
    ,.(max_churn = max(author_churn, na.rm = TRUE), n_windows = .N),
    by = author_name_email
    # Sort max churn in descending order and select the top n authors
][order(-max_churn)][1:min(top_n_authors, .N), author_name_email]
