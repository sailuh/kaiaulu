...
```{r}
file_graph <- transform_understand_dependencies_to_network(understand_parsed = file_dependencies, weight_types = c(keep_dependencies_type[2], keep_dependencies_type[5]))
...
```{r}
class_graph <- transform_understand_dependencies_to_network(understand_parsed = class_dependencies, weight_types = c(keep_dependencies_type[2], keep_dependencies_type[5]))

project_function_network <- igraph::graph_from_data_frame(d=class_graph[["edgelist"]],
                      directed = TRUE,
                      vertices = class_graph[["nodes"]])
visIgraph(project_function_network,randomSeed = 1)
```
