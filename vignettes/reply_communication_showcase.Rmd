...
jira_issue_comments_path <- conf[["issue_tracker"]][["issue_comments"]]
```


```{r}
project_mbox <- parse_mbox(perceval_path,mbox_path)
project_jira <- parse_jira(jira_issue_comments_path)[["comments"]]
```

Since Jira comments can be understood as e-wail relies, we can rename the specific columns of the comments table to conform it to project_mbox table. 

```{r}
project_jira <- project_jira[,.(reply_id=comment_id,
