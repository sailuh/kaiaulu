...
#' the necessary information regarding the JIRA issues and download
#' them to path designated save_path_issue_tracker_issues. In the future, a refresh 
#' capability will be added so that the user does not download the same issues multiple times
...
#' @param save_path_issue_tracker_issues the path that the files will be saved along. This is
...
fetch_and_save_jira_issues <- function(domain, 
  ...
  # List existing files in the save path. This part is commented out but not deleted as it is my first attempt at trying to implement the refresh function. It will be revisited later.
  # existing_files <- list.files(save_path_issue_tracker_issues, pattern = "\\.json$", full.names = TRUE)
  # issue_numbers <- sapply(existing_files, function(x) {
  #   matches <- regmatches(x, regexec("([A-Z]+-)([0-9]+)", basename(x)))
  #   as.numeric(matches[[1]][3])
  # })
  # 
  # # Find the largest issue number if any exist
  # if (length(issue_numbers) > 0) {
  #   max_issue_number <- max(issue_numbers, na.rm = TRUE)
  #   # Modify the JQL query to fetch only issues with a larger key
  #   jql_query <- paste(jql_query, "AND issue > ", max_issue_number)
  # }
...
jsonlite::write_json(all_issues_comments, save_p---
...
ath_issue_tracker_issue_comments)
