---
title: "Identifying GoF Patterns: Text and Graph Approach"
output: 
  html_document:
    toc: true
...
vignette: >
  ...
  %\VignetteIndexEntry{Identifying GoF Patterns: Text and Graph Approach}
  %\VignetteEncoding{UTF-8}
---

# Introduction

This notebook provides a brief introduction to the early version of the **Text** module, and Tsantalis pattern4.jar to identify some of the Gang of Four (GoF) Design Patterns in source code [pattern4.jar](https://users.encs.concordia.ca/~nikolaos/pattern_detection.html).
...
output_filepath <- conf[["tool"]][["pattern4"]][["output_filepath"]]
...
for(i in 1:length(topics)){
  topic_column_name <- paste0("is_topic_",as.character(i))
...
This approach uses Tsantali's [pattern4.jar](https://users.encs.concordia.ca/~nikolaos/pattern_detection.html) to identify GoF patterns.

```{r}
gof_patterns <- parse_gof_patterns(pattern4_path = pattern4_path,
...
#parse_last_list_element <- function(x){
#  return(tail(unlist(x),1))
#}

#is_method <- stringi::stri_detect_regex(gof_patterns$role_name,pattern="\\(\\)")

#gof_patterns_classes <- gof_patterns[!is_method]

#gof_patterns_classes$class_name <- sapply(stringi::stri_split_regex(gof_patterns_classes$class_path,pattern="\\."),parse_last_list_element)


gof_patterns[,.(class_path,pattern_name,role_name)] %>%
  gt(auto_align = FALSE) %>%
  tab_header(
    title = "GoF Patterns",
   ...
   cols_align(
    align = "right",
    columns = c("class_path")
  ) %>%
    cols_align(
    align = "left",
    columns = c("pattern_name","role_name")
  )
```
