...
git_branch <- conf[["git"]][["branch"]]

mbox_path <- conf[["data_path"]][["mbox"]]
github_replies_path <- path.expand(conf[["github"]][["replies"]])
...
mbox_datetimes <- project_reply$reply_datetimetz
...
for(j in 2:size_time_window){
  ...
  if(ml_exist){
    ...
    mbox_network_authors <- bipartite_graph_projection(network_reply_slice,
                                                      is_intermediate_projection = FALSE,
                                                      mode = TRUE)    
