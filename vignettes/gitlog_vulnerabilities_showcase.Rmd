...
Filter files based on the conf.yml to remove irrelevant files (such as .txt, .md, and .html), path contains the word "test", and exclude specific parts of the repository.
...
Copy of the original table with the filters above applied.
...
The "coverage" is the percentage of unique commits in the repository that contain a CVE-ID over the total number of unique CVE-ID. The propose is to get a overview of a repo's vulnerabilities.
...
The Common Weakness Enumeration (CWE) is a community-driven, standardized dictionary of software and hardware security weaknesses used to identify, classify, and prevent vulnerabilities. A "weakness" is a condition in software, firmware, hardware, or a service that under a certain condition/s, can lead to a vulnerability (https://cwe.mitre.org/about/index.html). The format is typically CWE-ID (e.g., CWE-79 or CWE-119). We can also graph the clusters, hubs, and bridges.

```{r}
cve_cwe_file_network <- transform_cve_cwe_file_to_network(project_commit_message_id_edgelist,cve_cwe_mapping)
```

The graph treats CVEs and CWEs as being in a similar group to one another.
