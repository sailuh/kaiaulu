...
graph_to_dsmj <- function(graph, dsmj_path, dsmj_name, is_directed, is_sorted=FALSE){
  ...
  format_dt_to_hdsmj_list <- function(grouped_cells){
    # ensure no 0 weight edges are recorded.
    grouped_cells <- grouped_cells[weight > 0]

    # If the pair of edges have at least one dependency type whose weight is not 0:
    if(nrow(grouped_cells) > 0){
      #s_grouped_cells is a data.frame of the form:
      # . Import Contain
      # . 1       1
      # More columns will be added depending on the dependency beng present.
      s_grouped_cells <- dcast(grouped_cells[,.(weight,label)],formula = . ~ label ,value.var = "weight")
      # Removes the first column named "." artificially generated by the dcast due to formula = . ~ label
      # E.g. now is:
      # Import Contain
      #  1       1
      s_grouped_cells <- s_grouped_cells[,2:ncol(s_grouped_cells),with=FALSE]
    }else{
      # No edge should be registered on the DSM (the row will still return but values = list())
      s_grouped_cells <- NULL
    }
    return(list(as.data.frame(s_grouped_cells)))

  }

  dsmj_list <- edgelist_table[,.(values = format_dt_to_hdsmj_list(.SD)),
                       by = .(from,to)]
